...
const StudioLayoutSample = ({ children }) => {
  const [sidebarOpen, setSidebarOpen] = useState(false);
  const location = useLocation();
  const navigate = useNavigate();

  const navigation = [
    { name: 'Dashboard', href: '/public/studio', icon: Home },
    { name: 'Create Experience', href: '/public/studio/create', icon: PlusCircle },
    { name: 'My Experiences', href: '/public/studio/experiences', icon: Music },
    { name: 'Analytics', href: '/public/studio/analytics', icon: BarChart3 },
    { name: 'Settings', href: '/public/studio/settings', icon: Settings },
  ];
      ...
      <div className={`fixed inset-y-0 z-50 flex flex-col w-64 transform ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'} transition-transform duration-300 ease-in-out bg-indigo-700 lg:translate-x-0 lg:static lg:inset-0`}>
        ...
        <div className="p-4 border-t border-indigo-600">
          <div className="flex items-center">
            ...
            <div className="ml-3">
              <p className="text-sm font-medium text-white">Public User</p>
              <p className="text-xs text-indigo-200">Demo Access</p>
            </div>
          </div>
            ...
            <div className="flex items-center space-x-4">
              ...
              <div className="relative">
                <div className="flex items-center">
                  ...
                  <span className="ml-2 text-sm font-medium text-gray-700 hidden md:block">Public Demo</span>
                </div>
              </div>
            </div>
          </div>
...
export default StudioLayoutSample;
